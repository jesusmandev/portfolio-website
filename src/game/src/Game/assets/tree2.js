    ...
    function growBranch(startPt, dir, length, baseRad, tipRad, level, angleTwist) {
        ...
        if (level >= 1) {
            ...
            for (let si = 0; si <= tubularSegments; si += sampleEvery) {
                ...
                canopySources.push({ pos: points[si].clone(), radius: 0.70 + sr * 5.0 });
            }
        ...
        if (level >= 0 && level < 3) {
            let numChildren = 0;
            if (level === 0) numChildren = Math.round(13 * branchiness);
            else if (level === 1) numChildren = Math.round(6 * branchiness);
            else if (level === 2) numChildren = Math.round(3 * branchiness);

            for (let k = 0; k < numChildren; k++) {
                const minT = level === 0 ? 0.30 : 0.1;
                ...
                const forwardBias = level === 0 ? 0.40 : 0.60;
                ...
                let lengthMultiplier = rand(0.55, 0.85);
                if (level === 0) {
                ...
                const childBaseRad = parentRadAtSpawn * rand(0.55, 0.82);
...
// Canopias ultra frondosas formadas por miles de hojas pequeñas.
// ============================================================================
const TREE_TYPE_CONFIGS = [
    {
        trunk: { height: 4.8, trunkRadius: 0.32, numRoots: 6, sway: 1.0, branchiness: 1.2 },
        foliage: { clumpCount: 85, planesPerClump: 72, planeSize: 0.32, scaleMin: 0.80, scaleMax: 1.25, radiusScale: 1.15 }
    },
    {
        trunk: { height: 7.5, trunkRadius: 0.26, numRoots: 5, sway: 0.8, branchiness: 1.0 },
        foliage: { clumpCount: 75, planesPerClump: 64, planeSize: 0.28, scaleMin: 0.75, scaleMax: 1.20, radiusScale: 1.05 }
    },
    {
        trunk: { height: 3.4, trunkRadius: 0.40, numRoots: 7, sway: 1.3, branchiness: 1.4 },
        foliage: { clumpCount: 95, planesPerClump: 78, planeSize: 0.35, scaleMin: 0.85, scaleMax: 1.35, radiusScale: 1.25 }
    },
    {
        trunk: { height: 5.8, trunkRadius: 0.28, numRoots: 6, sway: 0.9, branchiness: 1.1 },
        foliage: { clumpCount: 80, planesPerClump: 68, planeSize: 0.30, scaleMin: 0.80, scaleMax: 1.25, radiusScale: 1.10 }
    },
    {
        trunk: { height: 2.8, trunkRadius: 0.20, numRoots: 5, sway: 1.1, branchiness: 0.9 },
        foliage: { clumpCount: 65, planesPerClump: 56, planeSize: 0.26, scaleMin: 0.70, scaleMax: 1.10, radiusScale: 0.95 }
    }
];

// Paletas rosadas
export const TREE_PALETTES = [
    { shadow: 0xbe185d, mid: 0xdb2777, light: 0xf472b6, rim: 0xfbcfe8 },
    { shadow: 0x9d174d, mid: 0xc71585, light: 0xff69b4, rim: 0xffb6c1 },
    { shadow: 0x831843, mid: 0xdb2777, light: 0xf9a8d4, rim: 0xfce7f3 },
    { shadow: 0x86198f, mid: 0xec4899, light: 0xfda4af, rim: 0xfff1f2 },
];
...
export function generateTreeTemplate(seed, typeIndex = 0) {
    ...
    const sources = trunkRes.canopySources.length
        ? [...trunkRes.canopySources, ...trunkRes.endpoints.map(p => ({ pos: p, radius: 1.6 }))]
        : [{ pos: new THREE.Vector3(0, trunkRes.height * 0.85, 0), radius: 1.6 }];
...
function createFallingLeavesSystem(parent, foliageTexture, maxLeaves = 260) {
    ...
    const leafColors = [0xdb2777, 0xf472b6, 0xf9a8d4, 0xfbcfe8, 0xff69b4, 0xd63384];
