...
export const GridScan = ({
  ...
  return (
    <div ref={containerRef} className={`relative w-full h-full overflow-hidden ${className ?? ''}`} style={{ width: '100%', height: '100%', position: 'relative', ...style }}>
      {showPreview && (
        <div style={{ position: 'absolute', right: '12px', bottom: '12px', width: '220px', height: '132px', background: '#000', boxShadow: '0 4px 16px rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.25)', borderRadius: '8px', overflow: 'hidden', fontFamily: 'sans-serif', fontSize: '12px', color: '#fff', lineHeight: 1.2, pointerEvents: 'none' }}>
          <video ref={videoRef} muted playsInline autoPlay style={{ width: '100%', height: '100%', objectFit: 'cover', transform: 'scaleX(-1)' }} />
          <div style={{ position: 'absolute', top: '8px', left: '8px', background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', WebkitBackdropFilter: 'blur(4px)', padding: '2px 6px', borderRadius: '6px' }}>
            {enableWebcam
              ? modelsReady
                ? uiFaceActive
                  ? 'Face: tracking'
