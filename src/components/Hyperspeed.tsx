...
class App {
  ...
  loadAssets(): Promise<void> {
    ...
    return new Promise(resolve => {
      ...
      const searchURL = SMAAEffect.searchImageDataURL;
      const areaURL = SMAAEffect.areaImageDataURL;

      if (!searchURL || !areaURL) {
        console.warn('SMAA assets not found, skipping SMAA loading');
        resolve();
        return;
      }
      ...
      this.scene.traverse(object => {
        ...
        if (obj.material) {
          if (Array.isArray(obj.material)) {
            obj.material.forEach(material => (material as unknown as THREE.Material).dispose());
          } else {
...
const Hyperspeed = forwardRef<HyperspeedHandle, HyperspeedProps>(({ effectOptions = DEFAULT_EFFECT_OPTIONS }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    speedUp: () => appRef.current?.triggerSpeedUp(),
    slowDown: () => appRef.current?.triggerSlowDown()
  }));
